...
To achieve this, let’s add the following line to the `/etc/hosts` file on the AttackBox: `10.10.29.240 gift-scheduler.thm`
...
```shell-session
root@attackbox:~# echo "10.10.29.240 gift-scheduler.thm" >> /etc/hosts
```
...
10.10.29.240 gift-scheduler.thm
```
...
We must set the listening port to `8080` and toggle the `Specific address` option. The box next to it will automatically specify the IP address of our AttackBox, `10.10.90.235`. Finally, we can click on `OK` to apply the configuration.
...
Let’s add another line to the AttackBox’s `/etc/hosts` file. **Note:** The `10.10.90.235` address in the snippet should reflect the IP of our AttackBox, which can be found at the top of the page.

Terminal

```shell-session
root@attackbox:~# echo "10.10.90.235 wareville-gw" >> /etc/hosts
```
