...
Yes, it is absolutely possible to alter and manipulate the message and post using a different user ID.

All you need to do is:

1) Intercept the traffic in burp suite
2) go over to the website and use the input field to type something and send it over the network
3) the burp suite will capture the traffic and we can change the userId to 8 (Mayor Malware's id)  to send the message through mayor malware's client.
